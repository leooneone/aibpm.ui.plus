...
Sidebar.prototype.init = function()
{
	...
	//工作流默认图形(可定制交互)
	this.addWorkflowPalette(true);
	this.addGatewayPalette(true);
	// 基本组件
...
Sidebar.prototype.addWorkflowPalette = function(expand)
{
   ...
   // Avoids having to bind all functions to "this"
	...
	field.vertex = true;
	var fns = [
		this.createVertexTemplateEntry('shape=bpmShape;type=start;verticalLabelPosition=bottom;', 50, 50, '', '开始',null,'开始'),
		this.createVertexTemplateEntry('rounded=1;whiteSpace=wrap;html=1;type=approve', 120, 50, '', '审批',null,'审批'),
		this.createVertexTemplateEntry('rounded=1;whiteSpace=wrap;html=1;type=deal', 120, 50, '', '人工',null,'人工'),
		this.createVertexTemplateEntry('shape=bpmShape;type=end;verticalLabelPosition=bottom;', 50, 50, '', '结束',null,'结束'), 

		
		 ];

	this.addPaletteFunctions('workflow', mxResources.get('workflow'), (expand != null) ? expand : true, fns);
	this.setCurrentSearchEntryLibrary();
};
///增加网关组件
Sidebar.prototype.addGatewayPalette = function(expand)
{
	var lineTags = 'line lines connector connectors connection connections arrow arrows ';
	this.setCurrentSearchEntryLibrary('general', 'general');
   // Avoids having to bind all functions to "this"
	...
	var fns = [
		...
		this.createVertexTemplateEntry('shape=bpmShape;type=parallelGw;verticalLabelPosition=bottom;strokeColor=#0082cf;fillColor=#a6cce5', 50, 50, '', '并行',null,'并行网关'),
		this.createVertexTemplateEntry('shape=bpmShape;type=exclusiveGw;verticalLabelPosition=bottom;strokeColor=#0082cf;fillColor=#a6cce5', 50, 50, '', '排他',null,'排他网关'),
	//	this.createVertexTemplateEntry('shape=bpmShape;type=inclusiveGw;verticalLabelPosition=bottom;strokeColor=#0082cf;fillColor=#a6cce5', 50, 50, '', '包容',null,'包容网关'),

		
		 ];

	this.addPaletteFunctions('gateway', /*mxResources.get('gateway')*/'网关组件', (expand != null) ? expand : true, fns);
	this.setCurrentSearchEntryLibrary();
};
...
Sidebar.prototype.addGeneralPalette = function(expand)
{
   ...
   // Avoids having to bind all functions to "this"
	...
	field.vertex = true;
	var fns = [
		this.createVertexTemplateEntry('shape=bpmShape;type=Start', 50, 50, '', '开始',null,'开始'),
		...
		this.createVertexTemplateEntry('shape=switchBaseball;', 50, 50, '', '切换开关',null,'切换开关'),
		this.createVertexTemplateEntry('shape=switchMatrix;', 50, 50, '', '开关矩阵',null,'开关矩阵'),
...
b.forEach(item=>{
	fns.push(
		this.createVertexTemplateEntry(`image;;type=${item.iconName};html=1;image=/images/bpm/${key}/${item.iconName}.${item.fileType}`,
			 this.defaultImageWidth, this.defaultImageHeight, '', item.title, true, null, item.title)
	)
});
this.addPaletteFunctions(key, key, (expand != null) ? expand : true, fns);
...
Sidebar.prototype.addFoldingHandler = function(title, content, funct)
{
	...
	title.style.backgroundPosition = '4% 50%';
