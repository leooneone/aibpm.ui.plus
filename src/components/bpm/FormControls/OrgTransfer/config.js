...
export async function getRoles(params){
  ...
  const res = await new RoleApi().getListByParenId(params)
...
const defaultOption = {
  ...
  const DEP_CONFIG = Object.assign( {}, defaultOption )
//export const ROLE_CONFIG = Object.assign( {}, defaultOption, { tabKey: 'role', tabName: '角色' } )
  const USER_CONFIG = Object.assign( {}, defaultOption, { tabKey: 'user', tabName: '指定人员', onload: loadUserData, disabled: ( data, node ) => data.type!==( 'user' ) } )
const DEP_USER_CONFIG = Object.assign( {}, defaultOption, { tabKey: 'org&user', tabName: '组织和人员', onload: loadUserData, disabled: () => false } )
  const ROLE_CONFIG = Object.assign( {}, defaultOption, { tabKey: 'role', tabName: '角色' , onload: loadRoleData, disabled: ( data, node ) =>{
  return node.level ===1
}   } )
