        ...
        <div class="transfer-pane">
          ...
          <div class="transfer-pane__body" style="height: 370px;">
            ...
            <div
              ...
              <el-empty   v-if='searchRes.length===0' description="搜索结果为空"></el-empty>
              <div v-else v-for="(item, index) in searchRes" :key="index" class="item">
                <div>
            ...
            <el-scrollbar style="height:100%;">
              ...
              >
                ...
                <el-tab-pane v-for="(tab_item, idx) in tabConfig" :name="tab_item.tabKey" :label="tab_item.tabName" :key="idx">
                  ...
                  <span class="custom-tree-node"  scope-slot="{ node, data }">
                  <span>
                    <i :class="getIcon(data)"></i> {{ data.name }}
                  </span>
                  </span>
                  </el-tree>
                </el-tab-pane>
...
export default {
  ...
  methods: {
    getIcon(data) {
      if (data.type === 'user') { return 'el-icon-user-solid'; }
      if (data.type === 'org') {
        if (data.parentId === 0) return 'fa fa-building';
        return 'fa fa-sitemap';
      }

      if (data.type === 'role') {
        if (data.parentId === 0) return 'fa fa-address-book';
    ...
    searchDepUser() {
      ...
      new Promise((resolve, reject) => {
        activeConfig.onsearch(this.searchString, activeConfig.tabKey, resolve, reject);
      })
        .then((res) => {
    ...
    dataInit() {
      ...
      this.tabList.forEach((item) => {
        ...
        defaultConf && initDefaultData(key, { ...defaultConf, ...customConf });
      });
