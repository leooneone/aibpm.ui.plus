...
export class NodeUtils {
  ...
  ///isSub 来确定条件分支是否将当前节点下的所有子节点划入条件1作为其子节点
  static appendBranch(data, isBottomBtnOfBranch, isSub) {
    console.log(data)
    ...
    if (isSub) {
      ///将当前条件全部放在第一个条件中，新开一个条件
      conditionNodes[0].childNode = nodeData.childNode
      nodeData.childNode = undefined
    }

    // var sub=nodeData.childNode

    // nodeData.childNode.childNode=null
    // nodeData.childNode=sub.childNode
    // sub.childNode=null
    // conditionNodes[0].childNode=sub
  ...
  /**
* 新增线
* @param {Node[]} nodes  - 节点容器
* @param {Node} node - 待添加节点 
*/
  static addNode(nodes, node) {
    nodes[node.id] = (node)
...
* 移除节点
* @param {Node[]} nodes  - 节点容器
* @param {id} id - 待移除节点的id
*/
  static removeNode(nodes, id, lines) {
 ...
 */
  ...
  static checkNode(node, parent) {
    ...
    // 抄送人应该可以默认自选
    let valid = true
    const props = node.approve
    // !this.isStartNode( node )&& ( valid = false )

    // !this.isEndNode( node )&& ( valid = false )
    // && !props.initiator
    //&& ( valid = false )
    this.isConditionNode(node)
      && !node.isDefault
      && !node.initiator
      && isEmptyArray(node.conditions)
      && (valid = false)
    //form表示表单内的联系人组件
    const customSettings = ['myself', 'optional', 'director', 'supervisor', 'form', 'title']

    if (this.isApproverNode(node)) {
      if (!customSettings.includes(props.assigneeType) && isEmptyArray(props.participants))

        return { valid: false, msg: '参与人不能为空' }
      valid = false
    }
  ...
  static checkAllNode(processData) {
    ...
    const loop = (node, callback, parent) => {
      ...
      if (node.children) loop(node.children, callback, parent)
